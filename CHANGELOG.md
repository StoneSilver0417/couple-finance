...
- 원인: 월간 보고서(`generateMonthlyReport`)의 `asset_history` 조회에 **날짜 조건이 전혀 없었다**(`.order(record_date desc).limit(2)`뿐). 그래서 2026년 3월 같은 과거 월 보고서를 열어도 항상 오늘(최신) 순자산이 표시됐다. 기간 보고서(`generatePeriodicReport`)는 `record_date <= 기간말` 상한은 있었지만 "이전 순자산"이 같은 기간 안의 두 번째 스냅샷일 수 있어 전기간 대비 증감이 부정확했다.
- 수정: 두 경로 모두 순자산 조회를 두 건으로 분리했다. **순자산(current)**은 보고 기간 말(`record_date <= 기간 종료일`) 기준 최신 1건, **이전 순자산(previous)**은 기간 시작 전(`record_date < 기간 시작일`) 최신 1건으로 조회한다. 각각 `maybeSingle`로 받고, 해당 시점 이전 기록이 없으면 `null`로 처리해 증감·자산 코멘트를 자동 생략한다. 이로써 과거 월/기간 보고서는 그 시점의 순자산을, 전월/전기간 대비 증감은 기간 경계를 넘는 두 스냅샷으로 계산한다.
- 로컬 fallback 자산 코멘트 문구도 "최근 기록 기준 순자산은" → "{기간}까지 기록된 순자산은"으로 바꿔 과거 기간 보고서에서 "최근"이라는 오해가 없도록 했다.
- `npx tsc --noEmit`, `npx eslint`, `npm run build` 통과. 커밋 `c7d1812`. (실제 렌더 확인은 Gemini 키 등록 계정 필요.)

### feat - 분기·반기·연간 보고서 잔액/예산 표출을 연간 요약과 동일하게 통일

- 사용자 요청으로, 분기·반기·연간 AI 보고서의 요약 카드를 연간 요약 페이지(`/transactions/annual/[year]`)와 같은 방식으로 맞췄다.
- **잔액 → 월평균 잔액**: 기간 총잔액을 실제 기록이 있는 개월수로 나눈 월평균으로 표시하고 "N개월 기록 기준" 캡션을 병기한다. 나눗셈 기준 개월수는 기간 내 `monthly_balances` 로우 수(`periodMonthsWithData`)로, `generatePeriodicReport`에서 `stats`에 저장한다.
- **예산 사용률**: 퍼센트 아래에 "월평균 X원 지출"(기간 변동지출 ÷ 개월수) 캡션을 추가한다.
- `ReportView`는 `stats.periodMonthsWithData`가 있을 때만(=분기/반기/연간) 위 방식으로 전환한다. 월간 보고서는 이 필드를 설정하지 않아 기존 총액 표시를 그대로 유지한다(한 달짜리라 월평균이 총액과 같아 무의미). 기존에 저장된 보고서에 필드가 없어도 총액 표시로 안전하게 폴백한다.
- 보고서와 연간 요약 모두 `formatWon`(전체 원 표기)을 쓰므로 숫자 포맷도 이미 동일하다. `npx tsc --noEmit`, `npx eslint`, `npm run build` 통과. 커밋 `bb73e40`. (실제 렌더 확인은 Gemini 키 등록 계정 필요 — 기존 미검증 항목과 동일.)

### fix - 이전 기간 지출 기록이 없을 때 AI 보고서 허위 비교 방지

- 사용자가 "전월/전분기/전반기/전년 기록이 없을 때 대처가 없고, 대부분 이전 기록으로 비교 보고서를 써준다. 다르게 판단해야 할 것 같다"고 지적했다.
- 원인: `lib/report-actions.ts`의 월간·기간 보고서 두 경로 모두, 직전 기간에 지출 기록이 아예 없으면(`previousCategories`가 비어 있으면) `momCategoryDiffs` 계산에서 모든 현재 카테고리의 `prev`가 0이 되어 `diff = 현재 금액`(전부 양수)으로 잡혔다. 그 결과 "○○ 지출이 전분기보다 X원 늘었습니다"처럼 비교 대상이 없는데도 전부 증가로 서술됐고, "대비 주요 변화" 섹션에도 모든 항목이 증가로 표시됐다.
- 수정: 직전 기간 지출 기록 유무를 `hasComparisonBaseline = previousCategories.length > 0`로 판단해, 없으면 `momCategoryDiffs`를 빈 배열로 둔다. `ReportView`는 이미 `categoryDiffs.length > 0`일 때만 "대비 주요 변화" 섹션을 그리므로, 비교 데이터가 없으면 이 섹션이 자동으로 숨겨진다(허위 증가 표시 제거).
- Gemini 프롬프트에도 `hasComparisonBaseline` 플래그(`GeminiReportAggregates`에 신규 선택 필드)를 전달한다. 비교 기준이 없을 때는 시스템 인스트럭션이 "'전기간 대비' 같은 비교 표현을 절대 쓰지 말고 이번 기간의 지출 구성·규모 자체를 설명하라. momComments는 빈 배열로 두라"로 바뀐다. 기준이 있을 때는 기존 지시(순서 유지 비교)를 그대로 쓴다.
- 이전 기간에 지출 기록이 있으면 기존 비교 동작을 100% 유지한다(신규 카테고리는 증가로, 사라진 카테고리는 감소로 표시). 게이팅 로직을 Node 단언 스크립트로 검증(이전 없음→`hasComparisonBaseline=false`·diffs 빈 배열, 이전 있음→식비 +10만·교통 −5만·쇼핑 신규 +15만 정상)했고 `npx tsc --noEmit`, `npx eslint`, `npm run build` 통과.
...
- **마이그레이션 적용 및 배포 (Claude Code)**: 사용자가 `20260724000000_periodic_reports.sql`을 Supabase Dashboard SQL Editor에서 수동 적용 완료. 로컬 개발 서버와 프로덕션 양쪽에서 연간 요약 E2E 계정으로 로그인해 `/reports/2026`(연간)·`/reports/2026-Q3`(분기)·`/reports/2026-H1`(반기) 렌더와 기간 탭 전환, 잘못된 값(`/reports/abcd`)·미래 분기(`/reports/2027-Q1`) redirect를 직접 확인했다. 콘솔·서버 에러 0건으로 `periodic_reports` 테이블이 정상 생성됐음을 확인했고, 기존 `/reports/2026-07`(월간)도 동일 계정으로 재확인해 회귀가 없음을 검증한 뒤 커밋(`adc2bac`)·푸시·Vercel 배포(`Ready` 확인)까지 완료했다. 이 테스트 계정은 Gemini API 키가 미등록 상태라 실제 생성(Gemini 호출→저장) 경로는 검증하지 못했다 — 기존에도 있던 알려진 이슈로 남겨둠.
