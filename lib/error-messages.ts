...
const errorMap: Record<string, string> = {
  ...
  "violates unique constraint": "이미 존재하는 데이터입니다",
  ...
  "violates not-null constraint": "필수 입력 항목을 확인해주세요",
  ...
  "there is no unique or exclusion constraint matching the on conflict specification": "데이터 저장 설정 오류가 발생했습니다",
...
function parsePostgrestPatternError(message: string): string | null {
  const schemaColMatch = message.match(
    /could not find the '([^']+)' column of '([^']+)' in the schema cache/i,
  );
  if (schemaColMatch) {
    const [, column, table] = schemaColMatch;
    return `데이터베이스 스키마 오류: '${table}' 테이블에 '${column}' 컬럼이 존재하지 않습니다.`;
  }

  const colRelMatch = message.match(
    /column "?([^"\s]+)"? of relation "?([^"\s]+)"? does not exist/i,
  );
  if (colRelMatch) {
    const [, column, table] = colRelMatch;
    return `데이터베이스 스키마 오류: '${table}' 테이블에 '${column}' 컬럼이 존재하지 않습니다.`;
  }

  const colMatch = message.match(/column "?([^"\s]+)"? does not exist/i);
  if (colMatch) {
    const [, column] = colMatch;
    return `데이터베이스 스키마 오류: '${column}' 컬럼이 존재하지 않습니다.`;
  }

  const relMatch = message.match(/relation "?([^"\s]+)"? does not exist/i);
  if (relMatch) {
    const [, table] = relMatch;
    return `데이터베이스 스키마 오류: '${table}' 테이블이 존재하지 않습니다.`;
  }

  const fnMatch = message.match(
    /function ([a-zA-Z0-9_]+)\(.*?\)\s+does not exist/i,
  );
  if (fnMatch) {
    const [, fn] = fnMatch;
    return `데이터베이스 함수(RPC) 오류: '${fn}' 함수가 존재하지 않습니다.`;
  }
...
export function getKoreanErrorMessage(error: string | Error | unknown): string {
  ...
  const patternError = parsePostgrestPatternError(message);
  if (patternError) {
    return patternError;
  }
  ...
  for (const [key, value] of Object.entries(errorMap)) {
    if (key.length >= 5 && lowerMessage.includes(key.toLowerCase())) {
      return value;
    }
  }
