...
- **버전**: v0.6.4 + 월간 AI 보고서 구현·배포 완료, Gemini Auth key/무료 한도 완화 대응 반영 중
- **빌드 상태**: 성공 (`npx tsc --noEmit`, `npx eslint .` 0 오류·0 경고, `npm run build` 통과)
- **배포 상태**: 직전 프로덕션 배포 완료(2026-07-15, 문서 커밋 `9f04faf`, Vercel `dpl_EFStbE6zz6UXUYhWq2sJu96jd6HR` `Ready`). 이번 무료 한도 완화 변경은 커밋/배포 예정
- **프로덕션 URL**: https://couple-finance-roan.vercel.app
- **Supabase**: 개발/운영 분리 없이 `ieahmpxiaamesrnfgbng.supabase.co` 단일 프로젝트 사용

## 최근 작업

- **Gemini 무료 한도 완화 대응 (2026-07-15)**: 사용자가 다른 Google 계정의 신규 키를 적용해도 무료 한도 오류가 지속된다고 보고했다. 월간 보고서는 짧은 JSON 생성이라 기본 모델을 `gemini-2.0-flash`에서 `gemini-2.0-flash-lite`로 낮춰 한도 소모를 줄였고, 등록된 키가 있어도 삭제 없이 새 키로 교체 저장할 수 있도록 설정 다이얼로그에 교체 폼을 추가했다. 429 오류 문구도 AI Studio 프로젝트 한도 확인을 안내하도록 수정했다.
...
1. [ ] Gemini 신규 Auth key(`AQ...`) 교체 저장과 `gemini-2.0-flash-lite` 보고서 생성/재생성 E2E를 프로덕션에서 확인한다.
