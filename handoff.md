...
- **(완료) 월간 AI 보고서 기능 설계 + Codex 작업 지시서 작성 (2026-07-15)**: 구현은 Codex가 담당하기로 함. 지시서는 `docs/tasks/ai-monthly-report.md` — 요구사항(온디맨드 버튼 생성, 사용자 개인 Gemini 무료 키, 인앱 페이지), 설계 결정, 단계별 파일 목록, 참고 패턴 파일, 검증 체크리스트 포함. **주의**: 단계 1 마이그레이션 SQL은 사용자가 Supabase Dashboard SQL Editor에서 수동 실행해야 함
...
1. [ ] **월간 AI 보고서 구현 (Codex 담당)** — 지시서: `docs/tasks/ai-monthly-report.md`. 완료 검수는 Claude Code 세션에서 검증 체크리스트 기준으로
2. [ ] **[우선] 자산 변동 기록 스냅샷 미갱신 버그 조사·수정** (Supabase MCP로 `asset_history` 직접 조회 → RLS/upsert 원인 확정 → 수정)
3. [ ] **운영 DB 테스트 데이터 정리** (위 SQL 실행 후 Auth 계정 삭제)
4. [ ] 관리자 페이지 기능 고도화 (통계 등)
5. [ ] (필요시) Supabase Dashboard에서 SQL 마이그레이션 실행 (`20260212000000_security_rpc_fix.sql`)
